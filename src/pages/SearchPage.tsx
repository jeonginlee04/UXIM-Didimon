            ...
            {filtered.map((ann) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="text-4xl opacity-40">{THUMB_ICON[ann.category]}</span>
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleBookmark(ann.id) }}
                      className="absolute top-2 right-2 w-7 h-7 bg-white/80 rounded-full flex items-center justify-center touch-manipulation"
                    >
                      <Bookmark
                        size={14}
                        className={ann.isBookmarked ? 'text-primary' : 'text-text-disabled'}
                  ...
                  <div className="px-3 py-2.5">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] text-text-disabled">
                        {ann.startDate.slice(5).replace('-', '.')} - {ann.endDate.slice(5).replace('-', '.')}
                      </span>
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      <span
                        ...
                        style={{ color: catColor, backgroundColor: CATEGORY_BG[ann.category] }}
                      >
