...
export const useAnnouncementStore = create<AnnouncementState>()(
  persist(
    (set, get) => ({
      ...
      getTotalFilterCount: () => {
        const { filterCategory, filterDeadlines, filterRegions, filterBenefits, filterEligibility } = get()
        return (
      ...
      getFiltered: () => {
        const { announcements, searchQuery, filterCategory, filterDeadlines, filterRegions, filterBenefits, filterEligibility, sortBy } = get()
        ...
        if (filterDeadlines.length > 0) {
          ...
          result = result.filter((a) => {
            const diff = Math.ceil((new Date(a.endDate).getTime() - now) / 86400000)
        ...
        if (filterRegions.length > 0) {
          result = result.filter((a) =>
            a.region === '전국' || filterRegions.some((r) => a.region.includes(r))
          )
        }
        ...
        if (filterEligibility.length > 0) {
          result = result.filter((a) =>
            !a.targetAge || filterEligibility.some((e) => a.targetAge?.includes(e))
          )
        }
